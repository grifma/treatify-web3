...
contract DistributeEmbedded {
    ...
    function setSplit(uint[] memory _split) internal {
        validSplit(_split);
    ...
    function allocateAccounts() internal {
        for(uint i=0; i<accounts.length; i++) {
            uint thisAllocation = msg.value * split[accounts[i]] / 100 * 10 ** splitDecimals;
    ...
    //     for(uint i=0; i<accounts.length; i++) {
    //         uint thisAllocation = _value * split[accounts[i]] / 100 * 10 ** splitDecimals;
    //         emit Allocated(i, accounts[i], thisAllocation);
    //     }
    // }
    ...
    function manualERC20Distribution(address _tokenAddress) public returns (uint256) {
        for(uint i = 0; i < accounts.length; i++) {
            uint thisAllocation = IERC20(_tokenAddress).balanceOf(address(this)) * split[accounts[i]] / 100 * 10 ** splitDecimals;
